'use client';
import { toggleSvg } from '../../../internal/svg/toggleSvg.js';
import { cn, pressable, border } from '../../../styles/theme.js';
import { useAppchainBridgeContext } from './AppchainBridgeProvider.js';
import { jsx } from 'react/jsx-runtime';
function AppchainNetworkToggleButton({
  className
}) {
  const _useAppchainBridgeCon = useAppchainBridgeContext(),
    handleToggle = _useAppchainBridgeCon.handleToggle;
  return /*#__PURE__*/jsx("button", {
    type: "button",
    className: cn(pressable.alternate, border.default, 'flex h-14 w-14 items-center justify-center', 'rounded-lg border-4 border-solid', 'rotate-90', '-translate-x-1/2 absolute left-1/2 z-10', className),
    "data-testid": "AppchainNetworkToggleButton",
    onClick: handleToggle,
    children: toggleSvg
  });
}
export { AppchainNetworkToggleButton };
//# sourceMappingURL=AppchainNetworkToggleButton.js.map
